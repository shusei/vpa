import { fftRadix2, zeroCrossingRate } from "./audio-utils.js";
...
export function trimYinBuffers() {
...
export function detectPitchACF(input, sr, range) {
    ...
    const srDS = sr / ds;
    // const range = getPitchDetectorRange(); // Passed as arg
...
export function detectPitchYinLite(input, sr, range) {
    const ds = Math.max(1, Math.floor(sr / 16000));
    const N = Math.floor(input.length / ds);
    if (N < 128) return null;
...
export function estimateSpectralFeatures(frame, sr, range) {
    ...
    const f0 = detectPitchACF(frame, sr, range);
