...
- 當使用者發出指令 **「commit 要有 body 然後 push」**（或包含該意圖）時，必須 **100% 自動執行升版與發布流程**：
  1. **自動檢視變更**：讀取 `git diff` / `git status` 了解本次修改內容。
  2. **自動推算並升級版本號**：
     - 若為 Bug 修復或微調，升級 Patch 版本（例如 `1.1.0` ➔ `1.1.1`）。
     - 若為新功能或較大更新，升級 Minor 版本（例如 `1.1.0` ➔ `1.2.0`）。
  3. **自動同步更新檔案**：
     - 更新 `package.json` 中的 `version` 欄位。
     - 更新 `CHANGELOG.md`，新增對應版號的標題與條目摘要。
     - `CHANGELOG.md` 的分類標題與每個變更／測試條目必須同時提供英文及繁體中文，沿用既有雙語格式；同步至 GitHub Release 的說明也必須保留雙語，不得只寫英文。
